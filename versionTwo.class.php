...
class versionTwo
{
    ...
    public function checkFieldType($fieldStr){
        ...
        // 查找默认值
        $f_defaultExp = '/DEFAULT\s\'(.*?)\'(?:\sCOMMENT.*?)$/';
        ...
        // 查找注释
        $f_commentExp = '/COMMENT\s\'(.*?)\'$/';
        if (preg_match($f_commentExp, $fieldStr, $result)){
            $field_arr['Comment'] = $result[1];
        }else{
            $field_arr['Comment'] = '';
        }
